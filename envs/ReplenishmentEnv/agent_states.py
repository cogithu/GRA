...
class AgentStates(object):
    def __init__(
        ...
        self.replenishment_list = []
        for i in range(len(self.warehouse_list)):
            self.replenishment_list.append(np.zeros((
            self.durations + self.lookback_len,
            len(self.sku_ids),
            self.act_dim[i])))
        ...
        self.demand_list = []
        for i in range(len(self.warehouse_list)):
            self.demand_list.append(np.zeros((
            self.durations + self.lookback_len,
            len(self.sku_ids),
            self.sale_dim[i])))
        self.sale_list = []
        for i in range(len(self.warehouse_list)):
            self.sale_list.append(np.zeros((
            self.durations + self.lookback_len,
            len(self.sku_ids),
            self.sale_dim[i])))
        ...
        for warehouse_data in data:
            for item in self.states_items:
                if item in warehouse_data["dynamic_data"]:
                    ...
                    if item == 'demand':
                        demand = np.expand_dims(value, axis=-1)
                        self.__setitem__([self.warehouse_to_id[warehouse_data["warehouse_name"]], warehouse_data["warehouse_name"], item, "all_dates"], demand)
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index[0], int):
            if state_item == "replenish":
                if index[0] == -1:
                    value = []
                    for i in range(len(self.warehouse_list)):
                        value.append(self.replenishment_list[i][date])
                    return value
                else:
                    return self.replenishment_list[index[0]][date]
            if state_item == "demand":
                if index[0] == -1:
                    value = []
                    for i in range(len(self.warehouse_list)):
                        value.append(self.demand_list[i][date])
                    return value
                else:
                    return self.demand_list[index[0]][date]
            if state_item == "sale":
                if index[0] == -1:
                    value = []
                    for i in range(len(self.warehouse_list)):
                        value.append(self.sale_list[i][date])
                    return value
                else:
                    return self.sale_list[index[0]][date]
    ...
    def __setitem__(self, index, value):
        assert(isinstance(index, tuple) or isinstance(index, list))
        assert(len(index) >= 2 and len(index) <= 4)
        if np.isnan(value).all():  # 检查是否全部为 NaN
            value = np.nan_to_num(value, nan=0.0)  # 将整个数组替换为 0
        if isinstance(index[0], int):
            # if index[0] == -1:
            if len(index) == 3:
                warehouse_name = index[1]
        ...
        if isinstance(index[0], int):
            if state_item == "replenish":
                if index[0] == -1:
                    for i in range(len(self.warehouse_list)):
                        self.replenishment_list[i][date] = value[i]
                else:
                    self.replenishment_list[index[0]][date] = value
            if state_item == "demand":
                if index[0] == -1:
                    for i in range(len(self.warehouse_list)):
                        self.demand_list[i][date] = value[i]
                else:
                    self.demand_list[index[0]][date] = value
            if state_item == "sale":
                if index[0] == -1:
                    for i in range(len(self.warehouse_list)):
                        self.sale_list[i][date][date] = value[i]
                else:
                    self.sale_list[index[0]][date] = value
            return
        if state_item in self.states_items:
            state_index = self.states_to_id[state_item]
