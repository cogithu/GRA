...
class EmbedConv(MessagePassing):
    def __init__(self, 
        ...
        self.lin1 = nn.Sequential(
                        init_(nn.Linear(input_dim + embedding_size,
                                        hidden_size)),
                        active_func,
                        layer_norm,)
...
class TransformerConvNet(nn.Module):
    def __init__(self,
        ...
        self.learn_index = learn_index
        # self.max_edge_dist = max_edge_dist
        # self.graph_aggr = graph_aggr
        self.global_aggr_type = global_aggr_type
        index = []
        for i in range(len(self.learn_index)):
            for j in self.learn_index[i]:
                index.append(torch.tensor([j, i]))
        index = tuple(index)
        self.index = torch.stack(index, dim=0)
        ...
        self.embed_layer = EmbedConv(input_dim=input_dim-1, 
                            ...
                            edge_dim=edge_dim)
        self.gnn1 = TransformerConv(in_channels=input_dim,#embed_hidden_size,
    ...
    def forward(self, node_obs:Tensor, adj:Tensor):
        """
    ...
    def processAdj(self):
        ...
        # index = []
        # edge_attr = []
        # for i in range(len(learning_index)):
        #     for j in range(len(learning_index[i])):
        #         index.append([j, i])
                # edge_attr.append(reward[j])

        return self.index.transpose(1, 0).cuda()
