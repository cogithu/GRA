...
from algorithms.offpolicy.base.mlp_policy import MLPPolicy
from algorithms.offpolicy.maddpg.algorithm.actor_critic import MADDPG_Actor, MADDPG_Critic
# from algorithms.offpolicy.algorithms.matd3.algorithm.actor_critic import MATD3_Actor, MATD3_Critic
from algorithms.offpolicy.utils.util import is_discrete, is_multidiscrete, DecayThenFlatSchedule, get_dim_from_space, soft_update, hard_update, \
    gumbel_softmax, onehot_from_logits, gaussian_noise, avail_choose, to_numpy
from algorithms.offpolicy.utils.utils import print_box
...
class MADDPGPolicy(MLPPolicy):
    ...
    def __init__(self, config, policy_config, target_noise=None, td3=False, train=True):
        ...
        self.args = self.config["all_args"]
        ...
        actor_class = MADDPG_Actor
        critic_class = MADDPG_Critic
        ...
        if self.discrete:
            ...
            if explore:
                if np.random.uniform() < 0.1:
                    actions = np.random.uniform(0.0, 1.0,actor_out.shape)
                else:
                    actions = actor_out
                # actions = gaussian_noise(actor_out.shape, self.args.act_noise_std).cuda() + actor_out
