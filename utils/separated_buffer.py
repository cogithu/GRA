...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, share_obs_space, act_space, id):
        ...
        self.id = id
        self.learn_index = args.learn_index[id]
        ...
        if args.algorithm_name == "gra":
            self.share_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, args.num_agents, *obs_shape),
                                      dtype=np.float32)
        ...
        self.share_rewards = np.zeros((self.episode_length, self.n_rollout_threads, args.num_agents, 1),
                                      dtype=np.float32)
    ...
    def insert(self, share_obs, share_rewards, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        self.share_obs[self.step + 1] = share_obs.copy()
        self.share_rewards[self.step] = share_rewards.reshape(self.n_rollout_threads, self.num_agents, -1).copy()
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        #     rewards_local = rewards_local + self.share_rewards[:, :, i, :]
        # rewards_local = rewards_local/self.num_agents
    ...
    def compute_returns_gra(self, next_value, value_normalizer=None):
        rewards_local = self.rewards
        for i in self.learn_index:
            rewards_local = rewards_local + self.share_rewards[:, :, i, :]
        # rewards_local = rewards_local/len(self.learn_index)
        if self._use_proper_time_limits:
            if self._use_gae:
                self.value_preds[-1] = next_value
                gae = 0
            ...
            if self._use_gae:
                ...
                for step in reversed(range(self.rewards.shape[0])):
                    if self._use_popart or self._use_valuenorm:
                        delta = rewards_local[step] + self.gamma * value_normalizer.denormalize(self.value_preds[step + 1]) * self.masks[step + 1] - value_normalizer.denormalize(self.value_preds[step])
                        gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                        self.returns[step] = gae + value_normalizer.denormalize(self.value_preds[step])
                    else:
                        delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        share_rewards = self.share_rewards.reshape(-1, *self.share_rewards.shape[2:])
        ...
        for indices in sampler:
            ...
            share_rewards_batch = share_rewards[indices]
            ...
            yield share_obs_batch, obs_batch, share_rewards_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
